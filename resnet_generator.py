...
class ResNetV1(HybridBlock):
    ...
    def __init__(self, block, layers, channels, classes=10, thumbnail=True, **kwargs):
...
def get_resnet(version, num_layers, pretrained=False, ctx=gpu(),
    ...
    assert num_layers in resnet_spec, "Invalid number of layers: %d. Options are %s"%(
    ...
    net = resnet_class(block_class, layers, channels, 10, **kwargs)
